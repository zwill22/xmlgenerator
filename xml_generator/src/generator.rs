...
impl Generator {
    ...
    fn generate_xsd_type(&mut self, xsd_type: &XsdType) -> Option<String> {
        match xsd_type {
            ...
            XsdType::NegativeInt => fake_signed::<i32>(false, false),
            XsdType::NonNegativeInt => fake_signed::<i32>(true, true),
            XsdType::NonPositiveInt => fake_signed::<i32>(false, true),
            XsdType::PositiveInt => fake_signed::<i32>(true, false),
    ...
    fn generate_xsd_type_pattern(&mut self, xsd_type: &XsdType, pattern: &str) -> Option<String> {
        match xsd_type {
            XsdType::Byte => self.generate_regex(pattern),
            XsdType::Short => self.generate_regex(pattern),
            XsdType::Int => self.generate_regex(pattern),
            XsdType::Long => self.generate_regex(pattern),
            XsdType::NegativeInt => self.generate_regex(pattern),
            XsdType::NonNegativeInt => self.generate_regex(pattern),
            XsdType::NonPositiveInt => self.generate_regex(pattern),
            XsdType::PositiveInt => self.generate_regex(pattern),
            XsdType::UnsignedByte => self.generate_regex(pattern),
            XsdType::UnsignedShort => self.generate_regex(pattern),
            XsdType::UnsignedInt => self.generate_regex(pattern),
            XsdType::UnsignedLong => self.generate_regex(pattern),
            XsdType::Float => self.generate_regex(pattern),
            XsdType::Double => self.generate_regex(pattern),
            XsdType::URI => self.generate_samples(xsd_type, pattern),
            XsdType::Base64Binary => self.generate_samples(xsd_type, pattern),
            XsdType::Boolean => self.generate_regex(pattern),
            XsdType::HexBinary => self.generate_samples(xsd_type, pattern),
            XsdType::Duration => self.generate_regex(pattern),
