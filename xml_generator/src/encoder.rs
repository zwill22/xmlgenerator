...
fn filter(input: &str, whitespace: &WhiteSpace) -> Result<String, XMLGeneratorError> {
    ...
    for c in input.chars() {
        if c.is_whitespace() {
            ...
            return Err(XMLGeneratorError::UnimplementedFeature("Non characters".to_string()));
        } else if is_private_use(c) {
            return Err(
                XMLGeneratorError::UnimplementedFeature("Private use characters".to_string())
            );
        } else if c.age().is_none() {
            return Err(
                XMLGeneratorError::UnimplementedFeature("Unassigned Unicode characters".to_string())
            );
        } else {
            out.push(c);
        }
